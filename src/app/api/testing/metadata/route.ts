...
import { formatSize, detectSeriesByEpisodePattern, detectType, extractYear, extractSeason, extractQuality, extractLanguages, extractEpisodes, extractCleanTitle } from '@/lib/metadata-extractor';
...
function buildHeuristicMetadata(params: {
    ...
    const size = extractSizeFromTitle(combined) ? formatSize(extractSizeFromTitle(combined)!) : (extractSizeFromTitle(bodyText) ? formatSize(extractSizeFromTitle(bodyText)!) : null);
